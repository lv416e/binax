...
def compute_improved_reward(
    ...
    if (
        not opened_new_bin
        and new_bin_utilization[bin_idx] > state.bin_utilization[bin_idx]
    ):
...
def test_improved_rewards() -> None:
    ...
    def first_fit_action(state):
        ...
        for i, (capacity, utilization) in enumerate(
            zip(state.bin_capacities, state.bin_utilization, strict=False)
        ):
    ...
    while not state.done and step_count < 10:
        ...
        opened_new_bin = (old_utilization[bin_idx] == 0) & (
            next_state.bin_utilization[bin_idx] > 0
        )

        print(f"ステップ {step_count + 1}:")
        print(f"  アイテム {current_item_size:.3f} -> ビン {bin_idx}")
        print(
            f"  新ビン: {bool(opened_new_bin)}, 利用率: {next_state.bin_utilization[bin_idx]:.1%}"
        )
