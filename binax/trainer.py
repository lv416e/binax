...
from binax.networks import create_network, init_network_params
...
class Trainer:
    ...
    def collect_rollout(self, states: BinPackingState) -> Tuple[RolloutBatch, BinPackingState]:
        ...
        states_flat = jax.tree_map(lambda x: x.reshape(batch_size, *x.shape[2:]), states)
        actions_flat = BinPackingAction(
            bin_idx=actions.bin_idx.reshape(batch_size)
        )
